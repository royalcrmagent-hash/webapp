      ...
      {!isSuccess ? (
        <div className="flex-1 flex flex-col space-y-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 text-center space-y-2">
            <label className="text-xs text-slate-400 font-medium">Cash Out Amount</label>
            <div className="flex items-center justify-center gap-1">
              <span className="text-2xl font-bold text-amber-400">{wallet.currency}</span>
              <input
                type="number"
                value={amount}
            ...
            <p className="text-[11px] text-slate-400">Charge (1.5%): {wallet.currency}{(parseFloat(amount || '0') * 0.015).toFixed(2)}</p>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {['Withdrawal', 'Personal', 'Shopping', 'Emergency', 'Rent', 'Custom'].map((c) => (
                <button
                  key={c}
                  type="button"
          ...
          <div>
            <label className="text-xs font-semibold text-slate-400">Enter PIN</label>
            <input
              type="password"
              value={pin}
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-white">Cash Out Successful!</h2>
          <p className="text-xs text-slate-400">
            {wallet.currency}{parseFloat(amount).toLocaleString()} withdrawn from Agent {agentNumber}.
          </p>
          <button
            onClick={onBack}
